...
| `AMI_CACHE_MAX_ITEMS` | `400000` | upper bound on rows in the per-user vector cache, across all users, evicted least-recently-used. The cache is a read-through of SQLite, so eviction costs a reload and never a result. A dataset carrying one `user_id` per question would otherwise grow the process until it is killed |
